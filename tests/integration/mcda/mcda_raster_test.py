...
def test_sum_rasters(multi_criteria_vectors, debug: bool = False):
    ...
    all_criteria_vectors: list[tuple[str, str, gpd.GeoDataFrame]] = multi_criteria_vectors(max_value, min_value)
    ...
    if debug:
        # Using QGIS, it is easier doublecheck what values we are expecting in this test.
        for criterion in all_criteria_vectors:
            criterion_name, _, criterion_gdf = criterion
            criterion_gdf.to_file(Config.PATH_RESULTS / f"pytest_sum_{criterion_name}.geojson")
