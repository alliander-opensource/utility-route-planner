...
class TestMultiLayerRouting:
    ...
    def test_straightening_linestring_no_obstacle(self, setup_grid):
        ...
        assert len(route_engine.results.result_route_node_indices) == 20
        assert len(route_engine.results.result_route_straightened_node_indices) == 2
        # Due to the pointy top orientation, this is almost the same as the straightened line
        assert route_engine.results.result_route_linestring.length == pytest.approx(82.2, abs=0.1)
        assert route_engine.results.result_route_straightened.length == pytest.approx(82.2, abs=0.1)

        # Test that it can create a straight line: east -> west
        start_end = shapely.LineString([(10, 90), (90, 90)])
        route_engine.find_route(start_end)
        assert len(route_engine.results.result_route_node_indices) == 22
        assert route_engine.results.result_route_linestring.length == pytest.approx(90.9, abs=0.1)
        assert route_engine.results.result_route_straightened.length == pytest.approx(78.7, abs=0.1)
        assert len(route_engine.results.result_route_straightened_node_indices) == 2

        # test that it can create a straight line: diagonal
        start_end = shapely.LineString([(10, 90), (90, 10)])
        route_engine.find_route(start_end)
        assert len(route_engine.results.result_route_node_indices) == 30
        assert route_engine.results.result_route_linestring.length == pytest.approx(125.5, abs=0.1)
        assert route_engine.results.result_route_straightened.length == pytest.approx(112.3, abs=0.1)
        assert len(route_engine.results.result_route_straightened_node_indices) == 2
    ...
    def test_straightening_linestring_small_obstacle(self, setup_grid):
        ...
        route_engine.find_route(start_end)
        assert len(route_engine.results.result_route_node_indices) == 24
        assert route_engine.results.result_route_linestring.length == pytest.approx(99.6, abs=0.1)
        assert route_engine.results.result_route_straightened.length == pytest.approx(89.6, abs=0.1)
        assert len(route_engine.results.result_route_straightened_node_indices) == 3
    ...
    def test_straightening_linestring_large_obstacle(self, setup_grid):
        route_engine = setup_grid(
            (
                ...
                [10, 0, shapely.Point(91.022, 48.036).buffer(10)],  # smaller hurdle
            )
        )
        ...
        assert route_engine.results.result_route_linestring.length == pytest.approx(142.9, abs=0.1)
        assert route_engine.results.result_route_straightened.length == pytest.approx(130.8, abs=0.1)
        assert len(route_engine.results.result_route_node_indices) == 32
        assert len(route_engine.results.result_route_straightened_node_indices) == 9
    ...
    def test_multiple_segments_with_different_scores(self, setup_grid):
        route_engine = setup_grid(
            (
                [30, 0, shapely.Point(11.060, 9.058).buffer(4)],  # small round tower
                [29, 0, shapely.Point(25.969, 22.177).buffer(4)],  # small round tower
                [28, 0, shapely.Point(40.878, 35.295).buffer(4)],  # small round tower
                [27, 0, shapely.Point(56.787, 48.413).buffer(4)],  # small round tower
            )
        )
        route_engine.find_route(shapely.LineString([(10, 10), (60, 50)]))
