...
from utility_route_planner.models.multilayer_network.graph_datastructures import BaseWeightedEdgeInfo
...
class MultilayerRouteEngine:
    ...
    def find_route(self, start_end: shapely.LineString):
        ...
        if self.write_output:
            ...
            write_results_to_geopackage(self.out, self.line_target_source, f"{self.prefix}straight_line")
            write_results_to_geopackage(
                self.out,
    ...
    def get_weight(self, edge: BaseWeightedEdgeInfo, modifier: float = 0.01) -> float:
        """
            ...
            while shortcut_costs == basic_cost:
                ...
                shortcut_costs = (
                    self.gdf_cost_surface_nodes[
                        self.gdf_cost_surface_nodes.dwithin(linestring, self.hexagon_size * 0.50)
                    ]
                    .suitability_value.unique()
                    .tolist()
                )
